...
type
  ...
  published
    ...
    property LayerFont: TFont read fLayerFont
     write SetFont;
...
function ST: TSubtitlesForm;
begin
  case gST = NIL of TRUE: gST := TSubtitlesForm.create(NIL); end;
  result := gST;
end;
{$R *.dfm}
const
...
procedure TSubtitlesForm.FormCreate(Sender: TObject);
begin
  styleElements := [];
  // Initial settings
...
procedure TSubtitlesForm.FormDestroy(Sender: TObject);
begin
  FreeAndNil(fLayerFont);
end;

procedure TSubtitlesForm.SetFormPosition();
begin
...
procedure TSubtitlesForm.SetSubTitleText(aValue: string);
begin
  sSubTitleTxt := aValue;
  GetTextLines(aValue);
...
procedure TSubtitlesForm.GetTextLines(aValue: string);
var
  i: integer;
begin
...
begin
  hwParent := aVideoPanel.Handle;
end;

procedure TSubtitlesForm.CreateParams(var Params: TCreateParams);
begin
...
procedure TSubtitlesForm.SetFont(aValue: TFont);
begin
  Font := fLayerFont;
...
procedure TSubtitlesForm.SetBottomMargin(aValue: Integer);
begin
  iBottomMargin := aValue;
  lblSubTitle.Margins.Bottom := iBottomMargin;
end;

function TSubtitlesForm.GetScreenResolution(): TScrRes;
var
...
procedure TSubtitlesForm.OnParentChanged(var message: TMessage);
begin
  SetFormPosition();
end;

procedure TSubtitlesForm.CalculateFontSize();
const
  DEFAULTFONTSIZE = 100;
var
