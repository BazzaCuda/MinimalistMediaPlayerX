 ...
 *)
type
  P_mpv_node_list = ^mpv_node_list;
  P_mpv_byte_array = ^mpv_byte_array;

  mpv_node_u = record
    case Byte of
    1: (str: PMPVChar);   (* valid if format==MPV_FORMAT_STRING *)
    2: (flag: MPVInt);    (* valid if format==MPV_FORMAT_FLAG   *)
    3: (int64_: MPVInt64);   (* valid if format==MPV_FORMAT_INT64  *)
    4: (double_: Double); (* valid if format==MPV_FORMAT_DOUBLE *)
        (*
         * valid if format==MPV_FORMAT_NODE_ARRAY
         *    or if format==MPV_FORMAT_NODE_MAP
         *)
    5: (list: P_mpv_node_list);
        (*
         * valid if format==MPV_FORMAT_BYTE_ARRAY
         *)
    6: (ba: P_mpv_byte_array);
  end;
...
const
     ...
     *)
    MPV_EVENT_START_FILE_        = 6;
    (*
     * Notification after playback end (after the file was unloaded).
     * See also mpv_event and mpv_event_end_file.
     *)
    MPV_EVENT_END_FILE_          = 7;
    (*
...
type
  ...
  P_mpv_event_property = ^mpv_event_property;
...
type
  ...
  P_mpv_event_log_message = ^mpv_event_log_message;

/// Since API version 1.9.
type
  mpv_end_file_reason = MPVEnum;
...
type
  mpv_event_start_file = record
    (*
     * Playlist entry ID of the file being loaded now.
     *)
    playlist_entry_id: MPVInt64;
  end;
  P_mpv_event_start_file = ^mpv_event_start_file;
  ...
  mpv_event_end_file  = record
     ...
     * Since API version 1.108.
     *)
    playlist_insert_id: MPVInt64;
    (*
     * See playlist_insert_id. Only non-0 if playlist_insert_id is valid. Never
     * negative.
     * Since API version 1.108.
     *)
    playlist_insert_num_entries: MPVInt;
  end;
  P_mpv_event_end_file = ^mpv_event_end_file;
  ...
  P_mpv_event_client_message = ^mpv_event_client_message;

  mpv_event_hook = record
    (*
  ...
  P_mpv_event_hook = ^mpv_event_hook;
...
// Since API version 1.102.
  ...
  P_mpv_event_command = ^mpv_event_command;
